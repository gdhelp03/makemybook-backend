...
const getAllOrders = async (req, res) => {
  ...
  if (currentStep && currentStep !== "0") {
    console.log("yessssssss", currentStep)
    queryObject.currentStep = currentStep
  }
  ...
  try {
    ...
    const orders = await Order.find(queryObject)
      .select(
        "_id invoice paymentMethod subTotal total user_info discount shippingCost status createdAt updatedAt currentStep currentStepStatus"
      )
      .populate({
        path: "gallery",
        select: "name" // only bring back the gallery name
      })
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limits);
...
const updateOrder = async (req, res) => {
  ...
  // const currentStepStatus=req.body?.currentStepStatus
  ...
  const istNow = dayjs().tz("Asia/Kolkata").format(); // or custom format
  try {
    const { status, currentStep, currentStepStatus } = req.body;
  ...
  : null;
    ...
    if (currentStep != null && currentStep !== "") {
      updateFields.currentStep = currentStep;
      updateFields.currentStepStatus = currentStepStatus;

      // Update both root fields and the matching step in steps[]
      await Order.updateOne(
        { _id: mongoose.Types.ObjectId(orderId) },
        {
          $set: {
            ...updateFields,
            "steps.$[step].status": currentStepStatus,
            "steps.$[step].lastUpdatedTime": istNow,
            "steps.$[step].lastUpdatedBy": mongoose.Types.ObjectId(userId),
          },
        },
        {
          arrayFilters: [
            { "step.stepNumber": parseInt(currentStep) } // ensure it's a number
          ]
        }
      );
    } else {
      // If currentStep not provided, just update root fields
      await Order.updateOne(
        { _id: mongoose.Types.ObjectId(orderId) },
        { $set: updateFields }
      );
